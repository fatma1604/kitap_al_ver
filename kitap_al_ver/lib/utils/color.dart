// ignore_for_file: unnecessary_import

import 'dart:ui';

import 'package:flutter/material.dart';

final class AppColor {
  const AppColor._();
  static const Color lightBg = Color.fromARGB(232, 190, 126, 126);
  static const Color darttBg = Color.fromARGB(200, 134, 90, 90);

  static const Color onbordinglight = Color.fromARGB(255, 255, 172, 166);
  static const Color onboardingLightAccent = Color.fromARGB(255, 247, 118, 108);

  static const Color onbordingdark = Color.fromARGB(255, 119, 68, 65);
  static const Color onboardingDarkAccent = Color.fromARGB(230, 184, 118, 111);
//login-register-foget
  static const Color screenlight = Color.fromARGB(230, 199, 134, 130);
  static const Color screendart = Color.fromARGB(255, 95, 52, 50);

  static const Color fromlight = Color.fromARGB(169, 230, 112, 104);
  static const Color fromdart = Color.fromARGB(103, 77, 24, 21);

  static const Color forget = Color.fromARGB(255, 197, 121, 115);

//yazı
  static const Color textLiht = Color.fromARGB(244, 148, 80, 108);
  static const Color textDark = Color.fromARGB(221, 219, 216, 216);

//button
  static const Color buttonlight = Color.fromARGB(178, 248, 201, 201);
  static const Color buttondart = Color.fromARGB(206, 190, 141, 141);

  //ekranlar
  static const Color screenlight1 = Color.fromARGB(230, 219, 136, 132);
  static const Color screendart1 = Color.fromARGB(255, 116, 82, 80);

  static const Color screenlight2 = Color.fromARGB(255, 219, 123, 116);
  static const Color screendart2 = Color.fromARGB(255, 95, 52, 50);

  static const Color userTitle = Color.fromARGB(232, 243, 174, 174);

  static const Color white = Colors.white;
  static const Color black = Colors.black;
  static const Color shadow = Color.fromARGB(122, 185, 105, 138);

  static const Color liqudart = Color.fromARGB(255, 109, 64, 64);

  static const Color liquliht = Color.fromARGB(255, 247, 100, 100);

  static const Color icons = Color.fromARGB(214, 233, 224, 224);

  


  


}
